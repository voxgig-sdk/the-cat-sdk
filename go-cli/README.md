...
boru-driven command-line client **and** interactive REPL for the TheCat
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/the-cat-cli)
make build
...
./the-cat-cli list breed
./the-cat-cli list search

# 5. Override the API base URL for a single call
THE_CAT_BASE=https://api.example.com ./the-cat-cli list breed
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/the-cat-cli
   ```
...
Run with no arguments to open a REPL (prompt `the-cat>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `breed`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/the-cat-cli`. |
...
(`./the-cat-cli <expr>`) and interactively (the REPL), and expressions compose the
