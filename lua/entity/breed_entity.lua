...
local BreedEntity = {}
BreedEntity.__index = BreedEntity


function BreedEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BreedEntity)
  self._name = "breed"
  self._client = client
...
function BreedEntity:get_name()
  return self._name
end


function BreedEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BreedEntity.new(self._client, opts)
end
...
function BreedEntity:mark_deleted()
  self._deleted = true
end


function BreedEntity:deleted()
  return true == self._deleted
end


function BreedEntity:data_set(args)
  if args ~= nil then
...
function BreedEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BreedEntity:match_set(args)
  if args ~= nil then
...
function BreedEntity:match_get()
...
function BreedEntity:stream(action, args, callopts)
...
function BreedEntity:_run_op(ctx, post_done)
  local utility = self._utility
