...
class BreedEntity
  ...
  def make
    opts = @_entopts.dup
    BreedEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TheCatHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Breed, Hash] the current Breed data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TheCatHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Breed fields)
  def match_get
  ...
  #
  # @param reqmatch [BreedListMatch, Hash, nil] match filter (any subset of
  #   Breed fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Breed>, Array] the matching Breed items; raises TheCatError on failure
