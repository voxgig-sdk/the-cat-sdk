...
class BreedDirectTest < Minitest::Test
  def test_direct_list_breed
    setup = breed_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-breed", setup[:live] ? "live" : "unit")
    if _should_skip
...
def breed_direct_setup(mockres)
