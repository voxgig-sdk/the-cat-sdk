...
class FtCtrl extends TheCatControl
{
...
class FtClient
{
    ...
    private TheCatUtility $utility;

    public function __construct(TheCatUtility $utility, array $options)
    {
    ...
    public function get_utility(): TheCatUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TheCatContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new TheCatUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (TheCatContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TheCatConfig::make_feature($name);
    ...
    private static function buildUrl(TheCatSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TheCatError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TheCatResponse($fetched);
        }
    ...
    private function fail(TheCatContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TheCatContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
