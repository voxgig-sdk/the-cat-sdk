...
//    only - a typed TheCatContext there would make every ctx
...
final class TheCatOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new TheCatContext($clean, null);
    ...
    public static function err_from_map($m): ?TheCatError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new TheCatError(is_string($code) ? $code : '', $msg);
    }
