...
class TheCatTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TheCatTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TheCatTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TheCatTestRunner.getenv("THE_CAT_TEST_LIVE")
        override = TheCatTestRunner.getenv("THE_CAT_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TheCatTestRunner.getenv(key)
        ...
        explain = TheCatTestRunner.getenv("THE_CAT_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["THE_CAT_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TheCatTestRunner._test_control is not None:
            return TheCatTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TheCatTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TheCatTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TheCatTestRunner.load_test_control()
...
def load_env_local():
    TheCatTestRunner.load_env_local()


def env_override(m):
    return TheCatTestRunner.env_override(m)


def entity_data(v):
    return TheCatTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TheCatTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TheCatTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TheCatTestRunner.load_test_control()


def live_client_options():
    return TheCatTestRunner.live_client_options()


def live_delay_ms():
    return TheCatTestRunner.live_delay_ms()
