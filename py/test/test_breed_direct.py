...
class TestBreedDirect:

    def test_should_direct_list_breed(self):
        setup = _breed_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-breed", "live" if setup["live"] else "unit")
        if _skip:
...
def _breed_direct_setup(mockres):
