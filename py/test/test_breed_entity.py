...
class TestBreedEntity:
    ...
    def test_should_stream(self):
        ...
        base = TheCatSDK.test(seed, None)
        seen = list(base.Breed(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Breed(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _breed_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "breed." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set THE_CAT_TEST_BREED_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        breed_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.breed")))
        breed_ref01_data = None
        if len(breed_ref01_data_raw) > 0:
...
def _breed_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/breed/BreedTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("THE_CAT_TEST_BREED_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("THE_CAT_TEST_LIVE") == "TRUE":
        ...
        client = TheCatSDK(helpers.to_map(merged_opts))

    _live = env.get("THE_CAT_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("THE_CAT_TEST_EXPLAIN") == "TRUE",
        "live": _live,
