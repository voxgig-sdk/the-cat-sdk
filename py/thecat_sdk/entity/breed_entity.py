...
from thecat_sdk.thecat_types import (
    Breed,
    BreedListMatch,
)
...
class BreedEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return BreedEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Breed]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Breed().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
