...
class TheCatSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TheCatHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TheCatHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TheCatFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TheCatFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TheCatUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TheCatHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TheCatHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TheCatHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TheCatError($op . "_allow",
                "TheCatSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TheCatHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TheCatError("graphql_error",
                "TheCatSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Breed($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_breed === null) {
                $this->_breed = new BreedEntity($this, null);
            }
            return $this->_breed;
        }
        return new BreedEntity($this, $data);
    }
