...
{
    private static function utility(): TheCatUtility
    {
        return new TheCatUtility();
    }
    ...
    private static function ctx(array $over = []): TheCatContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TheCatContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TheCatOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TheCatError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TheCatOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TheCatMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TheCatOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TheCatMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TheCatOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TheCatMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TheCatSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TheCatMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TheCatResponse([]),
            'result' => new TheCatResult([]),
        ]);
        [, $err] = TheCatMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TheCatSpec(['step' => 's']),
            'response' => new TheCatResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TheCatResult([]),
        ]);
        [, $err] = TheCatMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TheCatSpec(['step' => 's']),
            'response' => new TheCatResponse(self::resp(200, ['v' => 1])),
            'result' => new TheCatResult([]),
        ]);
        [, $err] = TheCatMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TheCatSpec(['step' => 's']),
            'response' => new TheCatResponse(self::resp(200, ['v' => 2])),
            'result' => new TheCatResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TheCatMakeResponse::call($ctx);
    ...
    {
        $preset = new TheCatResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TheCatSpec([]),
            'response' => new TheCatResponse([]),
            'result' => new TheCatResult([]),
        ...
        [$got, $err] = TheCatMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TheCatResult([])]);
        [, $err] = TheCatMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TheCatSpec([]), 'result' => null]);
        [, $err] = TheCatMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TheCatSpec(['step' => 's']),
            'result' => new TheCatResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TheCatOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TheCatMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TheCatSpec(['step' => 's']),
            'result' => new TheCatResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TheCatOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TheCatMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TheCatResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TheCatSpec([]),
            'result' => new TheCatResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TheCatMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TheCatMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TheCatError('boom', 'boom');
        $utility->fetcher = function (TheCatContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TheCatSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TheCatMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TheCatContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TheCatMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TheCatResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TheCatContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TheCatSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TheCatMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TheCatResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TheCatSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TheCatMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TheCatMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TheCatMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TheCatResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TheCatDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TheCatResult(['ok' => false])]);
        $this->expectException(TheCatError::class);
        TheCatDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TheCatResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TheCatMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TheCatBaseFeature();
        $b = new TheCatBaseFeature();
        TheCatFeatureAdd::call($ctx, $a);
        TheCatFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TheCatBaseFeature
    {
        $f = new TheCatBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TheCatFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TheCatContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TheCatSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TheCatPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TheCatResponse(['headers' => null]),
            'result' => new TheCatResult([]),
        ]);
        TheCatResultHeaders::call($ctx);
