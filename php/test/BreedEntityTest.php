...
class BreedEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = TheCatSDK::test($seed, null);
        $seen = iterator_to_array($base->Breed(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = TheCatConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = TheCatSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Breed(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set THE_CAT_TEST_BREED_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function breed_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("THE_CAT_TEST_BREED_ENTID");
