...
// TheCat SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = TheCatOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(TheCatOmni::structfix($now, Struct::undef()))
                        !== json_encode(TheCatOmni::structfix($val, Struct::undef()))) {
