...
class TheCatNetsimFeature extends TheCatBaseFeature
{
    ...
    private function _simulate(TheCatContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TheCatContext $ctx, array $applied): void
    {
        $client = $this->client;
