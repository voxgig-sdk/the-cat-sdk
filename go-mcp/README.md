...
[MCP](https://modelcontextprotocol.io) server exposing the TheCat SDK as
two agent tools — `the-cat_list` and `the-cat_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/the-cat-mcp)
make build

# 2. Provide credentials via the environment
export THE_CAT_APIKEY=sk_live_xxx
...
```jsonc
// the-cat_list: first page of records
{ "entity": "breed" }
{ "entity": "breed", "query": { } }

// the-cat_load: one record by id
{ "entity": "breed", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/the-cat-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user the-cat \
     -- "$PWD"/dist/*/the-cat-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `the-cat_list` and `the-cat_load` tools now appear
   in new sessions. Ask the agent to *"list breed using the-cat"*
   and it calls `the-cat_list` with `{"entity":"breed"}`.
...
```sh
export THE_CAT_APIKEY=sk_live_xxx            # API key
export THE_CAT_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `the-cat_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `the-cat_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `THE_CAT_APIKEY` | API key sent with every request. |
| `THE_CAT_BASE` | Optional override of the API base URL. |
