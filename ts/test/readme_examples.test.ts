...
//     (\`new TheCatSDK(...)\` / \`TheCatSDK.test(...)\` become
//     \`TheCatSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"breed":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TheCatSDK(...)\` and any
// \`TheCatSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TheCatSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TheCatSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
