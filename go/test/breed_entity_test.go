...
func TestBreedEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Breed(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Breed(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set THE_CAT_TEST_BREED_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		breedRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.breed")))
...
func breedBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("THE_CAT_TEST_BREED_ENTID")
	...
	idmapResolved := core.ToMapAny(env["THE_CAT_TEST_BREED_ENTID"])
	...
	if env["THE_CAT_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewTheCatSDK(core.ToMapAny(mergedOpts))
	}
