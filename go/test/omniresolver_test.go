...
import (
	...
	omni "github.com/voxgig-sdk/the-cat-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TheCatError); is {
				out := map[string]any{
					"name":    "TheCatError",
...
func omniCtx(arg any, client *sdk.TheCatSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TheCatSDK); is {
			client = live
