...
import (
	"github.com/voxgig-sdk/the-cat-sdk/go/core"

	vs "github.com/voxgig-sdk/the-cat-sdk/go/utility/struct"
)

type BreedEntity struct {
	name    string
	client  *core.TheCatSDK
...
func NewBreedEntity(client *core.TheCatSDK, entopts map[string]any) *BreedEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *BreedEntity) GetName() string { return e.name }
...
func (e *BreedEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *BreedEntity) Deleted() bool {
	return e.deleted
}


func (e *BreedEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewBreedEntity(e.client, opts)
}

func (e *BreedEntity) Data(args ...any) any {
...
func (e *BreedEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Breed; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *BreedEntity) DataTyped(data ...Breed) Breed {
	if len(data) > 0 {
		return typedFrom[Breed](e.Data(asMap(data[0])))
	}
	return typedFrom[Breed](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Breed (all fields
// optional at the wire level).
func (e *BreedEntity) MatchTyped(match ...Breed) Breed {
	if len(match) > 0 {
		return typedFrom[Breed](e.Match(asMap(match[0])))
	}
	return typedFrom[Breed](e.Match())
}
...
func (e *BreedEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *BreedEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *BreedEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// BreedListMatch and returns []Breed. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *BreedEntity) ListTyped(reqmatch BreedListMatch, ctrl map[string]any) ([]Breed, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Breed](res), nil
}



func (e *BreedEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *BreedEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *BreedEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *BreedEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
