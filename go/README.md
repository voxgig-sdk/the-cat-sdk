...
The Golang SDK for the TheCat API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
func main() {
    ...
    // List breed records — the value is the array of records itself.
    breeds, err := client.Breed(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range breeds.([]any) {
        fmt.Println(item)
    }
}
```
...
fmt.Println(breeds) // the array of records
```
